...
VideoListTimeLine::VideoListTimeLine(QWidget *parent,int height)
      : QWidget(parent)
{
...
void VideoListTimeLine::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    emit mouseDoubleClickSignal(curSec);
}
...
void VideoListTimeLine::paint12minutesRuler()
{
    QPainter painter(this);
    painter.setRenderHint(QPainter::Antialiasing, true);
    painter.setPen(QPen(QColor(161, 161, 161), 2));

    double _12minuteWidth = timeLineWidth / 120.0;
    int _12minuteRulerBeginY = this->height() * 4/5+2 ;
    int _12minuteRulerEndY = this->height();
    /* 一天共有1440个1分钟 24 * 60 = 1440*/
    for (int i=0; i<120; i++)
    {
        double _12minuteRulerX = _12minuteWidth + _12minuteWidth * (i);
        painter.drawLine(QPointF(startPosX + _12minuteRulerX, _12minuteRulerBeginY),
                         QPointF(startPosX + _12minuteRulerX, _12minuteRulerEndY));
    }
