...
AllReplayWindow::AllReplayWindow(std::shared_ptr<VideoControl> vc, DeviceTree* devtree) :
    ...
    m_nameSet(std::make_shared<std::set<QString>>()), m_addCusChannel(std::make_shared<AddCustomizeChannel>(this)), m_devTree(devtree)
{
    ...
    m_windowingWgt->hide();
    m_videoControl->InitAllWindow(25, this, true);
    ...
    m_mainLayout->addWidget(m_vWidget, 747);
    m_mainLayout->addWidget(m_buttonWidget, 218);
...
void AllReplayWindow::resetShow(Video::VideoControlEnum i)
{
    m_videoControl->SetVideoControlEnum(i);
}

void AllReplayWindow::SaveCus()
{
    m_videoControl->WriteCusToDatabase();
}

void AllReplayWindow::mousePressEvent(QMouseEvent * event)
{
...
void AllReplayWindow::mouseMoveEvent(QMouseEvent * event)
{
    if (m_videoPressed)
    {
        m_videoMove = true;
...
void AllReplayWindow::mouseReleaseEvent(QMouseEvent * event)
{
    if (!m_videoMove)
    {
        m_selectChannelId = -1;
...
void AllReplayWindow::resizeEvent(QResizeEvent *size)
{
...
void AllReplayWindow::InitButton()
{
    m_buttonWidget->setStyleSheet("background-color: #EBEFF8");
    QVBoxLayout* mainLyt = new QVBoxLayout(m_buttonWidget);
    QWidget* leftBtnWidget = new QWidget;
    leftBtnWidget->setStyleSheet("background-color: #FFFFFF");
    QHBoxLayout* leftBtnLyt = new QHBoxLayout(leftBtnWidget);
    QWidget* rightBtnWidget = new QWidget;
    rightBtnWidget->setStyleSheet("background-color: #FFFFFF");
    QHBoxLayout* rightBtnLyt = new QHBoxLayout(rightBtnWidget);
    QHBoxLayout* topLyt = new QHBoxLayout;

    QPushButton* simulcastBtn = new QPushButton;
    SetPicBtnAttr(simulcastBtn, ":/img/tongbubofang.png", ":/img/tongbubofang2.png", tr("simulcast"));
    ...
    int width = simulcastBtn->width();
    leftBtnLyt->setSpacing(0);
    leftBtnLyt->addSpacing(2*width);
    leftBtnLyt->addWidget(simulcastBtn);
    leftBtnLyt->addSpacing(width);
    leftBtnLyt->addWidget(closeAllBtn);
    leftBtnLyt->addSpacing(2*width);
    ...
    connect(windowingBtn, &QPushButton::clicked, this, [this](){
        QPoint r(this->width(),this->height());
        QPoint gloPos = mapToGlobal(r);
        int w = gloPos.x();
        int h = gloPos.y();
        m_windowingWgt->move(w-m_windowingWgt->width()-60,h-m_windowingWgt->height()-60);
    ...
    rightBtnLyt->setSpacing(0);
    rightBtnLyt->addSpacing(2*width);
    rightBtnLyt->addWidget(windowingBtn);
    rightBtnLyt->addSpacing(width);
    rightBtnLyt->addWidget(fullWindowBtn);
    rightBtnLyt->addSpacing(2*width);

    VideoListTimeLine* timeLine = new VideoListTimeLine;
    leftBtnWidget->setFixedHeight(timeLine->height());
    rightBtnWidget->setFixedHeight(timeLine->height());
    topLyt->setSpacing(0);
    topLyt->addWidget(leftBtnWidget,160);
    topLyt->addSpacing(2);
    topLyt->addWidget(timeLine,1296);
    topLyt->addSpacing(2);
    topLyt->addWidget(rightBtnWidget,138);
...
void AllReplayWindow::CusBtnJudge()
{
...
void AllReplayWindow::AddCusBtn()
{
    m_addCusChannel->show();
}

/*
void AllReplayWindow::EditCusBtn()
{
...
void AllReplayWindow::SetPicBtnAttr(QPushButton* btn, const QString& picPath, const QString& hoveredPath, const QString& tipStr)
{
...
void AllReplayWindow::ButtonJudge(int buttonId)
{
    switch (buttonId)
    {
